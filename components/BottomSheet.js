...
export const BottomSheet = ({ show, children, title, onClose }) => {
    ...
    if (!show) return;
    return (
        <div className="block lg:hidden fixed h-full w-full bottom-0 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-50 z-50">
            <div
                className={`flex flex-col absolute bottom-0 bg-black w-full rounded-t-2xl slide-in-bottom border border-white border-opacity-40`}
            >
                ...
                <div className="flex items-center justify-between my-4 px-7">
                    <h1 className="text-xl leading-8 font-semibold text-white">
                        {title}
                    </h1>
